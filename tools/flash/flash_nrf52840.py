...
def open_session():
    session = ConnectHelper.session_with_chosen_probe(
        target_override=TARGET, blocking=False,
...
def cmd_info(args):
    with open_session() as session:
        ...
        print("DEVICEID    : %08X%08X" % (target.read32(FICR_DEVICEID1),
                                          target.read32(FICR_DEVICEID0)))
        ...
        print("slot0 header: 0x%08X%s" % (
            magic, "  (MCUboot image)" if magic == MCUBOOT_IMAGE_MAGIC else ""))
...
def cmd_bootstrap(args):
    ...
    with open_session() as session:
        ...
        print("slot0 image magic : 0x%08X%s"
              % (header, "  (MCUboot header present)"
                 if header == MCUBOOT_IMAGE_MAGIC else "  UNEXPECTED"))
    ...
    with open_session() as session:
        ...
        while time.time() - start < args.seconds:
            ...
            if target.get_state().name == "HALTED":
                print("%5.1fs  FATAL: stopped at PC=0x%08X"
