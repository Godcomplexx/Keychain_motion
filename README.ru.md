...
- Откат обновления реализован, но **на железе ещё не проверен**: SMP-сервер в
  приложении собран, маршрут через запасной слот написан, а испытать его без
  подключённой платы нельзя. До проверки считать эту возможность заявленной, а
  не работающей.
- Заряд измеряется по напряжению, и точнее этот метод быть не может: в середине
  кривой LiPo сто милливольт меняют заряд вдвое. Поэтому на экране рядом с
  индикатором выводятся вольты — измеряется именно они, — а во время зарядки
  показывается анимация заполнения без числа, потому что под током напряжение
  приподнято и любой процент был бы завышен.
