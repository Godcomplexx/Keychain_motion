...
приложении. `Start Breakout` по-прежнему открывает игру.
...
В приложении три кнопки: `Start auto sync`, `Show time` и `Start Breakout`.
...
3. Брелок переключается на экран часов на 60 секунд; радио остаётся включённым,
