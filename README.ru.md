...
| наклонили влево / вправо | взгляд плавно уходит в сторону и возвращается | P1 |
| встряхнули | глаза круглые, персонаж подпрыгивает | P1 |
| встряхнули второй раз за 5 с | брови к центру, короткая дрожь | P1 |
...
| дольше | одно самостоятельное занятие | P3 |
...
**Анимация частиц FLIP никуда не делась**, но стоит быть точной в том, что это
значит. Она стала одним из занятий питомца, когда его оставили в покое, наряду
с разглядыванием звёзд и блужданием взгляда.

*Как её увидеть:* она всегда идёт первым занятием после включения. Включите
плату и не трогайте её секунд двадцать. Дальше три занятия чередуются, и выбрать
конкретное нельзя — жеста или команды, вызывающей частицы по требованию, сейчас
нет.

*Зависимость от акселерометра осталась.* Без датчика частицы получают
синтетическую пилу наклона: они движутся, но ни на что не реагируют — это
демонстрация, а не поведение версии 1. Припайка LIS2DW12 возвращает настоящий
наклон без изменения прошивки.
...
Синхронизация времени с Android-приложением и Breakout с управлением наклоном
остались от версии 1 без изменений.
...
Кнопки RESET на плате нет, поэтому в DFU не входят удержанием чего-либо.
Приложение получает касание 1200 бод на своём USB-порту, пишет флаг режима
загрузки в регистр, переживающий сброс, а MCUboot читает его и вместо запуска
приложения поднимает serial recovery.
...
- Отката у обновления по USB нет. Serial recovery в MCUboot — механизм
  восстановления, а не обновления: он не ведёт состояния слотов, поэтому
  пометка «тестовый образ» ничего не меняет, и запись что в основной слот, что
  в запасной даёт сразу постоянный образ. Проверено на плате. Настоящий откат
  требует SMP-сервера в самом приложении; половина для этого уже есть —
  приложение вызывает `boot_write_img_confirmed()` при успешном старте.
- Индикатор заряда показывает напряжение, а не заряд. Под током зарядки
  напряжение на ячейке приподнято, поэтому во время зарядки процент завышен
  примерно на восемь пунктов. Показания сглажены, но систематическую ошибку это
  не убирает: честный уровень виден только с отключённым кабелем.
- Метод в принципе чувствителен: в середине кривой LiPo сто милливольт меняют
  заряд вдвое, поэтому проценты в диапазоне 30–60% стоит считать оценкой, а не
  измерением.
...
3. Встряхните брелок три раза.
4. Приложение находит `KeychainSync`, отправляет время телефона и отключается.

Фоновая синхронизация использует энергоэффективный режим сканирования BLE.
Явные ручные команды используют более быстрый сбалансированный режим.

### Breakout

1. Нажмите `Start Breakout`.
2. Встряхните брелок три раза.
3. Телефон отправляет `GAME:START`.
4. BLE выключается, игра продолжается локально.
5. Наклоняйте брелок влево и вправо, чтобы двигать платформу.
...
Текущие материалы показывают концепцию взаимодействия и направление корпуса.
Фотографии собранной электроники появятся после интеграции корпуса.
...
- Добавить автоматические host-тесты для состояний, поведения и игры.
